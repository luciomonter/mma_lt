...
export class AddonAxificationsProvider {
     ...
     */
    protected formatAxificationsData(notifications: any[]): void {
        ...
        return this.sitesProvider.getSite(siteId).then((site) => {
            ...
            return site.read('core_message_get_messages', data, preSets).then((response) => {
                if (response.messages) {
                    ...
                    this.formatAxificationsData(notifications);
                    if (this.appProvider.isDesktop() && toDisplay && !read && limitFrom === 0) {
                        ...
                        this.emulatorHelper.storeLastReceivedAxification(
                            AddonAxificationsProvider.PUSH_SIMULATION_COMPONENT, notifications[0], siteId);
                    }
     ...
     * @returns {Promise<any>} Resolved when done.
     */
    markAxificationRead(notificationId: number): Promise<any> {
        const params = {
