...
/**
 * Axifications cron handler.
 */
@Injectable()
export class AddonAxificationsCronHandler implements CoreCronHandler {
    name = 'AddonAxificationsCronHandler';
    ...
    constructor(private appProvider: CoreAppProvider, private eventsProvider: CoreEventsProvider,
            ...
            private axificationsProvider: AddonAxificationsProvider, private textUtils: CoreTextUtilsProvider,
    ...
    isSync(): boolean {
        ...
        return !this.axificationsProvider.isPreciseAxificationCountEnabled() || this.appProvider.isDesktop();
    }
    ...
    execute(siteId?: string): Promise<any> {
        if (this.sitesProvider.isCurrentSite(siteId)) {
            this.eventsProvider.trigger(AddonAxificationsProvider.READ_CRON_EVENT, {}, this.sitesProvider.getCurrentSiteId());
        }

        if (this.appProvider.isDesktop() && this.localNotifications.isAvailable()) {
            this.emulatorHelper.checkNewNotifications(
                AddonAxificationsProvider.PUSH_SIMULATION_COMPONENT,
                this.fetchAxifications.bind(this), this.getTitleAndText.bind(this), siteId);
        }
    ...
    /**
     * Get the latest unread axifications from a site.
     *
     * @param  {string} siteId  Site ID.
     * @return {Promise<any[]>} Promise resolved with the axifications.
     */
    protected fetchAxifications(siteId: string): Promise<any[]> {
        return this.axificationsProvider.getUnreadAxifications(0, undefined, true, false, true, siteId);
    }
     ...
     */
    protected getTitleAndText(axification: any): Promise<any> {
        const data = {
            title: axification.userfromfullname,
            text: axification.mobiletext.replace(/-{4,}/ig, '')
        };
